...
export default function LeaderboardTable() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        setTraders(
          data.sort((a: TraderData, b: TraderData) => b.amount - a.amount)
        );
        setLoading(false);
      } catch (error) {
  ...
  return (
    <div
      className={`${pixelFont.className} bg-white bg-opacity-90 border-2 border-[#808080] shadow-inner`}
    >
            ...
            traders.map((trader, index) => (
              ...
              >
                <td className="py-1 px-4 flex items-center gap-2">
                  {getRankDisplay(index + 1, trader.amount)}
                </td>
                <td className="py-1 px-4">{trader.name}</td>
                <td className="py-1 px-4 text-right">
                  $
                  {trader.amount.toLocaleString(undefined, {
      ...
      {loading && (
        <div className="py-4 text-center animate-pulse">LOADING TRADERS...</div>
      )}
    </div>
  );
}

function getRankDisplay(rank: number, amount: number) {
  if (amount <= 0.01) {
    return <span className="text-red-900">†</span>;
  }
